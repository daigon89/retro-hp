...
interface SortState {
  key: keyof PreRow | null;
  dir: SortDir;
}

const COLUMNS: { key: keyof PreRow; label: string }[] = [
  { key: "プレNo", label: "プレNo" },
  { key: "アポNo", label: "アポNo" },
  ...
  { key: "アポ日", label: "アポ日" },
  { key: "プレ予定日", label: "プレ予定日" },
  { key: "プレ予定時間", label: "プレ予定時間" },
  { key: "姓", label: "姓" },
  { key: "名", label: "名" },
  { key: "メールアドレス", label: "メールアドレス" },
  { key: "営業担当者", label: "営業担当者" },
  { key: "プレステータス", label: "プレステータス" },
  { key: "成約要因", label: "成約要因" },
  { key: "失注要因", label: "失注要因" },
...
// プレステータスの色分け
const PRE_STATUS_COLORS: Record<string, string> = {
  ...
  プレ済: "bg-blue-100 text-blue-800",
};

function StatusBadge({ status }: { status: string }) {
  if (!status) return <span className="text-gray-400">-</span>;
  const colorClass = PRE_STATUS_COLORS[status] ?? "bg-purple-100 text-purple-800";
  return (
    <span
...
function sortRows(rows: PreRow[], key: keyof PreRow | null, dir: SortDir): PreRow[] {
...
function filterRows(rows: PreRow[], query: string): PreRow[] {
...
export default function PreTable({ rows }: PreTableProps) {
  const [sort, setSort] = useState<SortState>({ key: null, dir: null });
  const [searchInput, setSearchInput] = useState("");
  const debouncedSearch = useDebounce(searchInput, 300);

  function handleHeaderClick(key: keyof PreRow) {
    setSort((prev) => {

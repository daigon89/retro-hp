...
import { ApoRow } from "@/lib/sheets";

type SortDir = "asc" | "desc" | null;

interface SortState {
  key: keyof ApoRow | null;
  dir: SortDir;
}

const COLUMNS: { key: keyof ApoRow; label: string; numeric?: boolean }[] = [
  { key: "アポNo", label: "アポNo" },
  { key: "導線種別", label: "導線種別" },
  { key: "申込日", label: "申込日" },
...
// アポステータスの完全な色分け
const APO_STATUS_COLORS: Record<string, string> = {
...
function StatusBadge({ status }: { status: string }) {
  ...
  const colorClass = APO_STATUS_COLORS[status] ?? "bg-purple-100 text-purple-800";
  return (
    <span
...
function sortRows(rows: ApoRow[], key: keyof ApoRow | null, dir: SortDir): ApoRow[] {
...
function filterRows(rows: ApoRow[], query: string): ApoRow[] {
...
interface ApoTableProps {
  rows: ApoRow[];
}

export default function ApoTable({ rows }: ApoTableProps) {
  const [sort, setSort] = useState<SortState>({ key: null, dir: null });
  const [searchInput, setSearchInput] = useState("");
  const debouncedSearch = useDebounce(searchInput, 300);

  function handleHeaderClick(key: keyof ApoRow) {
    setSort((prev) => {

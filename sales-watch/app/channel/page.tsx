...
import { computeChannelSummaries } from "@/lib/summaryUtils";
...
export default async function ChannelPage({ searchParams }: PageProps) {
  ...
  const { rows, total } = computeChannelSummaries(apoRows, contractRows, ym, preRows);
  ...
  return (
    <main className="max-w-screen-2xl mx-auto px-4 py-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <h1 className="text-xl font-bold text-slate-800">導線種別ビュー</h1>
      ...
      <p className="text-sm text-slate-500 mb-4">
        ...
        　導線種別数: <span className="font-medium text-slate-700">{rows.length}件</span>
        　<span className="text-slate-400">（▶ で展開してイベント別サブテーブルを表示）</span>
      </p>

      <ChannelTable rows={rows} total={total} />
    </main>
  );
}
